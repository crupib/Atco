...
struct s_FsBuffer *FsBuffer;
struct s_SoundBuffer *SoundBuffer;
struct s_Mod *Mod;
...
void loadHeader() {
 ...
 FlashSeek(0);
 for(i = 0; i < 20; i++)
  Mod->name[i] = FlashGetc();

 for(i = 0; i < SAMPLES; i++) {
  for(j = 0; j < 22; j++)
   Mod->samples[i].name[j] = FlashGetc();
  Mod->samples[i].length = word(FlashGetc(), FlashGetc()) * 2;
  Mod->samples[i].fineTune = FlashGetc();
  if(Mod->samples[i].fineTune > 7) Mod->samples[i].fineTune -= 16;
  Mod->samples[i].volume = FlashGetc();
  Mod->samples[i].loopBegin = word(FlashGetc(), FlashGetc()) * 2;
  Mod->samples[i].loopLength = word(FlashGetc(), FlashGetc()) * 2;
 ...
 Mod->songLength = FlashGetc();
 FlashGetc(); // Discard this byte

 Mod->numberOfPatterns = 0;
 for(i = 0; i < 128; i++) {
  Mod->order[i] = FlashGetc();
...
void loadPattern(uint8_t pattern) {
 ...
 FlashSeek(1084 + pattern * ROWS * Mod->numberOfChannels * 4);
...
void mixer() {
 ...
 for(channel = 0; channel < Mod->numberOfChannels; channel++) {
  ...
  if(samplePointer < FsBuffer->samplePointer[channel] ||
     samplePointer >= FsBuffer->samplePointer[channel] + FSBUFFERSIZE - 1 ||
     Mixer->channelSampleNumber[channel] != FsBuffer->channelSampleNumber[channel]) {

   fsBufferSize = Mixer->sampleEnd[Mixer->channelSampleNumber[channel]] - samplePointer + 1;
   if(fsBufferSize > FSBUFFERSIZE) fsBufferSize = FSBUFFERSIZE;

   FlashSeek(samplePointer);
   for(i = 0; i < fsBufferSize; i++)
    FsBuffer->channels[channel][i] = FlashGetc();

   FsBuffer->samplePointer[channel] = samplePointer;
   FsBuffer->channelSampleNumber[channel] = Mixer->channelSampleNumber[channel];
  }

  current = FsBuffer->channels[channel][(samplePointer - FsBuffer->samplePointer[channel]) & (FSBUFFERSIZE - 1)];
  next = FsBuffer->channels[channel][(samplePointer + 1 - FsBuffer->samplePointer[channel]) & (FSBUFFERSIZE - 1)];
...
void loadMod() {
 ...
 for(channel = 0; channel < Mod->numberOfChannels; channel++) {
  ...
  FsBuffer->samplePointer[channel] = 0;
  FsBuffer->channelSampleNumber[channel] = 0xFF;
