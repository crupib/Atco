...
const float rad2deg = 3.14159 / 180;				// Rad to Degree

Obj3D	Object3D[MaxObj]	__attribute__((section(".GFX_3DOBJ_Section")));
zBuff3D	zBuffer[MaxFce]		__attribute__((section(".GFX_3DOBJ_Section")));
float	zBuffVal[MaxFce]	__attribute__((section(".GFX_3DOBJ_Section")));
...
// Init the Object 0 in case we start with a higher object number
void Init3d(void)
{
	uint32_t cnt;
	for(cnt = 0;cnt < MaxObj;cnt++){
		Object3D[cnt].Angle.x = 0;
		Object3D[cnt].Angle.y = 0;
		Object3D[cnt].Angle.z = 0;
		Object3D[cnt].Zoom = 1.0;
		Object3D[cnt].Coord.x = 0.0;
		Object3D[cnt].Coord.y = 0.0;
		Object3D[cnt].Coord.z = 0.0;
		Object3D[cnt].NbPoint = 0;
		Object3D[cnt].NbFace = 0;
		Object3D[cnt].Loaded = 0;
		Object3D[cnt].Active = 0;
	}
	for(cnt = 0;cnt < MaxFce;cnt++){
		zBuffer[cnt].p1 = 0;
		zBuffer[cnt].p2 = 0;
		zBuffer[cnt].p3 = 0;
		zBuffer[cnt].incolor = 0;
		zBuffer[cnt].outcolor = 0;
		zBuffer[cnt].objnum = 0;
		zBuffVal[cnt] = 0.0;
	}
}
...
void Compute3D(void){
	...
	for(numobj = 0 ; numobj < MaxObj ; numobj++)
		if((Object3D[numobj].Loaded == 1) & (Object3D[numobj].Active == 1)){
			ax = Object3D[numobj].Angle.x;
			ay = Object3D[numobj].Angle.y;
			az = Object3D[numobj].Angle.z;

			sx = calcsi[ax]; sy = calcsi[ay]; sz = calcsi[az];
			cx = calcco[ax]; cy = calcco[ay]; cz = calcco[az];

			//       Compute the X,Y,Z rotation
			for(a = 1; a <= Object3D[numobj].NbPoint ; a++)
			{
				//		Rotation on X Axis
				tmpy = Object3D[numobj].Pts[a].y * cx - Object3D[numobj].Pts[a].z * sx;
				tmpz = Object3D[numobj].Pts[a].y * sx + Object3D[numobj].Pts[a].z * cx;
				Object3D[numobj].CalcPts[a].y = tmpy;
				Object3D[numobj].CalcPts[a].z = tmpz;
				//		Rotation on Z Axis
				tmpx = Object3D[numobj].Pts[a].x * cz - Object3D[numobj].CalcPts[a].y * sz;
				tmpy = Object3D[numobj].Pts[a].x * sz + Object3D[numobj].CalcPts[a].y * cz;
				Object3D[numobj].CalcPts[a].x = tmpx;
				Object3D[numobj].CalcPts[a].y = tmpy;
				//		Rotation on Y Axis
				tmpx = Object3D[numobj].CalcPts[a].x * cy - Object3D[numobj].CalcPts[a].z * sy;
				tmpz = Object3D[numobj].CalcPts[a].x * sy + Object3D[numobj].CalcPts[a].z * cy;
				Object3D[numobj].CalcPts[a].x = tmpx;
				Object3D[numobj].CalcPts[a].z = tmpz;
				//		Point Translation
				Object3D[numobj].CalcPts[a].x = (Object3D[numobj].CalcPts[a].x * Object3D[numobj].Zoom) + Object3D[numobj].Coord.x;
				Object3D[numobj].CalcPts[a].y = (Object3D[numobj].CalcPts[a].y * Object3D[numobj].Zoom) + Object3D[numobj].Coord.y;
				Object3D[numobj].CalcPts[a].z = (Object3D[numobj].CalcPts[a].z * Object3D[numobj].Zoom) + Object3D[numobj].Coord.z;
			}
		}
}
...
void zBuff(uint16_t nb){
	...
	//    Compute Z value from each face
	for(a = 1 ; a <= nb ; a++)
		zBuffVal[a] = (	Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p1].z +
						Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p2].z +
						Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p3].z) / 3;
	//    Sort the Face from back to front
	for(a = 1 ; a <= (nb - 1) ; a++)
		for(b = a + 1 ;b  <= nb ; b++)
			if( zBuffVal[b] < zBuffVal[a])
			{
				tmpz1 		= zBuffVal[a];
				tmpf 		= zBuffer[a];

				zBuffVal[a] = zBuffVal[b];
				zBuffVal[b] = tmpz1;
				zBuffer[a] 	= zBuffer[b];
				zBuffer[b] 	= tmpf;
			};
...
void Cache3D(void){
	...
	// Search for hidden Face
	for(numobj = 0 ; numobj < MaxObj ; numobj++)
		if((Object3D[numobj].Loaded == 1) & (Object3D[numobj].Active == 1))
			for(a = 1 ; a <= Object3D[numobj].NbFace ; a++)
			{
				v1 = Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p1].x - Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p2].x;
				v2 = Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p3].y - Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p2].y;
				v3 = Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p3].x - Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p2].x;
				v4 = Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p1].y - Object3D[numobj].CalcPts[Object3D[numobj].Face[a].p2].y;
				facevector = (v1 * v2) - (v3 * v4);
				if(facevector < 0)
				{
					VisibleFaces++;
					zBuffer[VisibleFaces].p1		= Object3D[numobj].Face[a].p1;
					zBuffer[VisibleFaces].p2		= Object3D[numobj].Face[a].p2;
					zBuffer[VisibleFaces].p3		= Object3D[numobj].Face[a].p3;
					zBuffer[VisibleFaces].incolor	= Object3D[numobj].Face[a].incolor;
					zBuffer[VisibleFaces].outcolor	= Object3D[numobj].Face[a].outcolor;
					zBuffer[VisibleFaces].objnum	= numobj;
				}
			}
	zBuff(VisibleFaces);
}
...
//--------------------------------------------------------------
// Load one 3D Objects from File
// filename		: Name from the file to load
// ObjectNum	: Number from the 3D Object
// FirstPoint	: Index from the First point of the 3D Object
// FirstFace	: Index from the First Face of the 3D Object
//--------------------------------------------------------------
// Returned Values: 1 = Wrong object number
//					2 = Too much points or faces in this object
//					3 = File open error
uint8_t Load_3DObj(const char* filename , uint16_t ObjectNum, uint16_t incolor, uint16_t outcolor)
{
	// 3D Object TextFile information
	// First value is the number of Points in the Object
	// Second valus is the number of Faces in the Object
	// Then comes all x,y,z from points
	// And all p1,p2,p3 from the Faces
	// Convertion of Data (All data from file are signed 32 bits)
	// byte 1 stay as is
	// byte 2 << 8
	// byte 3 << 16
	// byte 4 << 24
	uint32_t 	cnt  = 0;					// Generic counter
	uint32_t	PointCount = 0;				// Number of 3D Point to read
	uint32_t	FaceCount  = 0;				// Number of 3D Face  to read
	...
	Object3D[ObjectNum].Loaded = 0;

	// Read number of points in the 3D object
	b1 = MMfgetc(fn);			// NB Point
	b2 = MMfgetc(fn);			// NB Point
	b3 = MMfgetc(fn);			// NB Point
	b4 = MMfgetc(fn);			// NB Point
	PointCount = b1 + (b2 << 8) + (b3 << 16) + (b4 << 24);
	// Read number of faces in the 3D object
	b1 = MMfgetc(fn);			// NB Faces
	b2 = MMfgetc(fn);			// NB Faces
	b3 = MMfgetc(fn);			// NB Point
	b4 = MMfgetc(fn);			// NB Point
	FaceCount = b1 + (b2 << 8) + (b3 << 16) + (b4 << 24);


	if((PointCount >= Max3DPoint) | (FaceCount >= Max3DFace)){
		MMfclose(fn);
		return 2;
	}
	//Record number of points and faces into the Object Variable
	Object3D[ObjectNum].NbPoint = PointCount;
	Object3D[ObjectNum].NbFace  = FaceCount;

	// Read all 3D points from the object
	// All 3D Coordinates have to be divided by 10000
	cnt = 1;
	while(cnt <= Object3D[ObjectNum].NbPoint)
	{
		// Coord X from the point
		b1 = MMfgetc(fn);			// Point Coord X
		b2 = MMfgetc(fn);			// Point Coord X
		b3 = MMfgetc(fn);			// Point Coord X
		b4 = MMfgetc(fn);			// Point Coord X
		Object3D[ObjectNum].Pts[cnt].x = (b1 + (b2 << 8) + (b3 << 16) + (b4 << 24)) / 10000.0;
		// Coord Y from the point
		b1 = MMfgetc(fn);			// Point Coord Y
		b2 = MMfgetc(fn);			// Point Coord Y
		b3 = MMfgetc(fn);			// Point Coord Y
		b4 = MMfgetc(fn);			// Point Coord Y
		Object3D[ObjectNum].Pts[cnt].y = (b1 + (b2 << 8) + (b3 << 16) + (b4 << 24)) / 10000.0;
		// Coord Z from the point
		b1 = MMfgetc(fn);			// Point Coord Z
		b2 = MMfgetc(fn);			// Point Coord Z
		b3 = MMfgetc(fn);			// Point Coord Z
		b4 = MMfgetc(fn);			// Point Coord Z
		Object3D[ObjectNum].Pts[cnt].z = (b1 + (b2 << 8) + (b3 << 16) + (b4 << 24)) / 10000.0;
		cnt++;
	}


	// Read all faces from the 3D object
	cnt = 1;
	while(cnt <= Object3D[ObjectNum].NbFace)
	{
		// Coord X from the point
		b1 = MMfgetc(fn);			// Face p1
		b2 = MMfgetc(fn);			// Face p1
		b3 = MMfgetc(fn);			// Face p1
		b4 = MMfgetc(fn);			// Face p1
		Object3D[ObjectNum].Face[cnt].p1 = b1 + (b2 << 8) + (b3 << 16) + (b4 << 24);
		// Coord Y from the point
		b1 = MMfgetc(fn);			// Face p2
		b2 = MMfgetc(fn);			// Face p2
		b3 = MMfgetc(fn);			// Face p2
		b4 = MMfgetc(fn);			// Face p2
		Object3D[ObjectNum].Face[cnt].p2 = b1 + (b2 << 8) + (b3 << 16) + (b4 << 24);
		// Coord Z from the point
		b1 = MMfgetc(fn);			// Face p3
		b2 = MMfgetc(fn);			// Face p3
		b3 = MMfgetc(fn);			// Face p3
		b4 = MMfgetc(fn);			// Face p3
		Object3D[ObjectNum].Face[cnt].p3 = b1 + (b2 << 8) + (b3 << 16) + (b4 << 24);
		Object3D[ObjectNum].Face[cnt].incolor = incolor;
		Object3D[ObjectNum].Face[cnt].outcolor = outcolor;
		cnt++;
	}
	Object3D[ObjectNum].Loaded = 1;
	MMfclose(fn);
	return 0;
}
...
void Show_3DObject(uint8_t Filled , uint8_t BuffNum){
	...
	// Compute Angle , Translation and Zoom
	Compute3D();

	// Compute invisible face and zBuffer
	Cache3D();

	  for(a = 1 ; a <= VisibleFaces ; a++)
	      {
	      points.ax = Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p1].x;
	      points.ay = Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p1].y;
	      points.bx = Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p2].x;
	      points.by = Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p2].y;
	      points.cx = Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p3].x;
	      points.cy = Object3D[zBuffer[a].objnum].CalcPts[zBuffer[a].p3].y;
	      if(Filled == 1)
	    	  GFX_Draw_Full_Triangle(points,zBuffer[a].incolor,zBuffer[a].outcolor,BuffNum);
	      else
	    	  GFX_Draw_Triangle(points,zBuffer[a].outcolor,BuffNum);
	      };
}
