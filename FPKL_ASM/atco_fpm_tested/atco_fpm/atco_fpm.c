					 ...
					 unsigned char* bitmap8bit, int bmwidth, int bmheight)
{
	...
	if (*ipm == FP_PLOT_MODE_ERASE_AND_PLOT  ||  *ipm == FP_PLOT_MODE_PLOT_ONLY  ||  *ipm == FP_PLOT_MODE_ACCUMULATE)
	{
        ...
        prevy = 0, prevy2 = 0;
		t = 0;
		reciprocalscale = ((__int64)(double)(1LL << 32) * (double)(1LL << 32) / (double)(sp->iscale));
		...
		for (i = 0; i < sp->n; i++)
			if (sp->iscale >= (1LL << 32))
			{
				...
				x1 = curx >> 32;
				y1 = getUnipolar(sp, buff, x1);
				y = (((_int64)(getUnipolar(sp, buff, x1+1) - y1) * (curx & ((1LL << 32) - 1))) >> 32) + y1;
			...
			else
			{
				...
				do
				{
					y = getUnipolar(sp, buff, t++);
					miny = min(miny, y);
					maxy = max(maxy, y);
					curx += sp->iscale;
				} while (curx >> 32  ==  x1);
				y1 = min(prevy2+1, miny);
				y2 = max(prevy-1, maxy);
