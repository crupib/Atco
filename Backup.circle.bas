...
FUNCTION circle(CenterX AS LONG, CenterY AS LONG, Radius AS DOUBLE ) AS LONG
    LOCAL x,y AS DOUBLE
    LOCAL k AS DOUBLE
...
FUNCTION Calculate_Radius_Arc_AB() AS LONG
  ...
  Central_Angle = (Arc_AB / Circumference) * 360.00
...
FUNCTION BUILDWINDOW() AS LONG
    ...
    TXT$ = "Scale"
    CONTROL ADD LABEL, hDlg, %IDC_LABSPIN, TXT$, 700, 420, 20, 20
    ...
    CONTROL ADD TEXTBOX, hDlg, %IDC_EDITBOX3, "", 800, 210, 80, 20, , , _
    CALL EditControlCallback() ' Use default styles
    ...
    CONTROL ADD OPTION, hDlg, %OPT3, "Radius & Segment Height ED", 800, 34, 180, 14 , _
     , , CALL Button_call()
    CONTROL ADD OPTION, hDlg, %OPT4, "Radius & Apothem OE", 800, 48, 180, 14   , _
     , , CALL Button_call()
    CONTROL ADD OPTION, hDlg, %OPT5, "Radius & Arc AB", 800, 62, 180, 14  , _
     , , CALL Button_call()
