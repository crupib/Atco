...
// generate a integer random number that is greater than or equal to 0 but less than 65535
void fun_irnd(void) {
	fret = (float)(Random_Get() % 65535);
}

void fun_acos(void){
	fret = acosf(getnumber(ep));
}

void fun_asin(void){
	fret = asinf(getnumber(ep));
}

void fun_calcco(void){
	fret = calcco[(uint16_t)getnumber(ep) % 360];
}

void fun_calcsi(void){
	fret = calcsi[(uint16_t)getnumber(ep) % 360];
}

void fun_fwidth(void){
	fret = fontWidth * fontScale;
}

void fun_fheight(void){
	fret = fontHeight * fontScale;
}

//---------------------------------
// old Maximite functions
//---------------------------------
