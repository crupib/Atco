...
void cmd_edit(void) {
    	char *fromp, *p;
    	int i, y, x;
    	int OldBgColour, oldfont;


    	if(CurrentLinePtr) error("Invalid in a program");
    	ClearRuntime();

    	OldBgColour=DefaultBgColour;
    	DefaultBgColour=CurTxtBgColour;
    	oldfont=GetFlashOption(&FontOption);

    	SetFont(DEFAULT_FONT-1, 1, 0); // set a reasonable default font
    ...
    //    		StartEditPoint = findline(i = getinteger(cmdline), true);
    	...
    	DefaultBgColour=OldBgColour;
    	SetFont(oldfont-1, 1, 0); // set font
    	MMcls();
}
...
void MarkMode(char *cb, char *buf) {
	...
	while(1) {
		...
		if(mark < txtp) {
			PositionCursor(mark);
			SetFont(DEFAULT_FONT-1, 1, 1); // reverse video
			VT100Send("\033[7m");
			p = mark;
			while(p < txtp) {
				if(*p == '\n') MMputchar('\r');
...
#endif
	MMline(0, VHeight * (fontHeight * fontScale) + (fontHeight * fontScale)/2, (VWidth * (fontWidth * fontScale)) - 1, VHeight * (fontHeight * fontScale) + (fontHeight * fontScale)/2, CurTxtFgColour);
	SCursor(0, VHeight + 1);
	for(i = 0; i < VWidth; i++)
		if(*p)
			MMputchar(*p++);
...
void ScrollDown(void) {
	...
	// because the dma2d can't copy backwards
	// we need to copy first in a "working-buffer"
	// and then back into the screen-buffer

	// copy from source- to working-buffer
	...
	if(LCD_CurrentLayer==0) {
		...
		dst_addr = GFX_FRAME_BUFFER+(2 * BUFFER_OFFSET); // working buffer
	}
	...
	// copy from working- to destination-buffer
	if(LCD_CurrentLayer==0) {
		src_addr = GFX_FRAME_BUFFER+(2 * BUFFER_OFFSET); // working buffer
		dst_addr = GFX_FRAME_BUFFER+((HRes*2)*lines);
	}
	else if(LCD_CurrentLayer==1) {
		src_addr = GFX_FRAME_BUFFER+(2 * BUFFER_OFFSET); // working buffer
		dst_addr = GFX_FRAME_BUFFER+BUFFER_OFFSET+((HRes*2)*lines);
	}
	else return;
	GFX_CopyBuffer(src_addr,dst_addr,HRes,split,0,0);

	// print first line at the top of the screen
	edy--;
	j = VideoOn;
	SCursor(0, 0);
	printLine(edy);
