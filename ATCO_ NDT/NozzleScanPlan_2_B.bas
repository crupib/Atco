...
SUB DFWriteArray (filenum AS INTEGER,  passrec() AS EXT , OFFSET AS LONG)
        PUT filenum,  offset , PASSREC()
END SUB

SUB DFReadArray (filenum AS INTEGER, passrec() AS EXT , OFFSET AS LONG)
...
'        sYpos(n%) = (yStroke##*SIN(thetaRads##))+eYpos(n%) 'get upper yPos: based on angle and stroke length
 ...
 '    NEXT
    ...
    CALL DFReadArray (filenumber,BYVAL VARPTR(eXpos()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eYpos()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eNormXpos()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eNormYpos()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eNormAngle()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eRotAngle()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eNormRad()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
 '   CALL DFReadArray (filenumber,BYVAL VARPTR(eChord()) , myoffset)
 '   myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eArcSeg()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(eArcTotal()) , myoffset)
    myoffset = myoffset +(extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(sXpos()) , myoffset)
    myoffset = myoffset + (extsize*mydimsize)
    CALL DFReadArray (filenumber,BYVAL VARPTR(sYpos()) , myoffset)

'    myoffset = 0
'    CALL DFWriteArray( filenumber,BYVAL VARPTR(eXpos()) ,myoffset)
'    myoffset = myoffset + (extsize*mydimsize)
'    CALL DFWriteArray( filenumber,BYVAL VARPTR(eYpos()) ,myoffset)
'    myoffset = myoffset + (extsize*mydimsize)
'    CALL DFWriteArray( filenumber,BYVAL VARPTR(eNormXpos()) ,myoffset)
'    myoffset = myoffset +(extsize*mydimsize)
'    CALL DFWriteArray( filenumber,BYVAL VARPTR(eNormYpos()) ,myoffset)
'    myoffset = myoffset + (extsize*mydimsize)
'    CALL DFWriteArray( filenumber,BYVAL VARPTR(eNormAngle()) ,myoffset)
'    myoffset = myoffset + (extsize*mydimsize)
'    CALL DFWriteArray( filenumber,BYVAL VARPTR(eRotAngle()) ,myoffset)
'    myoffset = myoffset + (extsize*mydimsize)
'    CALL DFWriteArray( filenumber,BYVAL VARPTR(eNormRad()) ,myoffset)
'    myoffset = myoffset + (extsize*mydimsize)
 '   CALL DFWriteArray( filenumber,BYVAL VARPTR(eChord()) ,myoffset)
 '   myoffset = myoffset + (extsize*mydimsize)
 '   CALL DFWriteArray( filenumber,BYVAL VARPTR(eArcSeg()) ,myoffset)
 '   myoffset = myoffset + (extsize*mydimsize)
 '   CALL DFWriteArray( filenumber,BYVAL VARPTR(eArcTotal()) ,myoffset)
 '   myoffset = myoffset + (extsize*mydimsize)
 '   CALL DFWriteArray( filenumber,BYVAL VARPTR(sXpos()) ,myoffset)
 '   myoffset = myoffset + (extsize*mydimsize)
 '   CALL DFWriteArray( filenumber,BYVAL VARPTR(sYpos()) ,myoffset)
