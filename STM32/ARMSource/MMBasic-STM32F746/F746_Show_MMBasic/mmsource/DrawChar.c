...
extern unsigned short C64_8x8_Table[];     // Font from UB
extern unsigned short Arial_8x13b_Table[];  // Font from UB
extern unsigned short Arial_10x15b_Table[];  // Font from UB
...
// Write a char on the video using the graphic bit font (with Font from UB)
void DrawCharUB(int x, int y, int c, int mode)
{
	...
	for(j = 0; j < fontHeight; j++) {        						// Loop through each word
		for(k = 0; k < fontWidth; k++) {         				// Loop through the bits in the word
			if(fontWidth <= 8) {
			  t = ((*(fontPtr16 + ((c - fontStart) * fontHeight) + j)) & (1<<(fontWidth-k))) >> (fontWidth-k);
			}
			else {
			  t = ((*(fontPtr16 + ((c - fontStart) * fontHeight) + j)) & (1<<(16-k))) >> (16-k);
			}
			if(fontReverse) t = !t;
			if(mode) {
				if(mode & 0b010) t = !t;
			...
			for(h = 0; h < fontScale; h++)
				for(w = 0; w < fontScale; w++)
					if(t)
						plotx(x + (k * fontScale) + w, y + (j * fontScale) + h, CurTxtFgColour);	// Draw the foreground colour
					else
						plotx(x + (k * fontScale) + w, y + (j * fontScale) + h, CurTxtBgColour);	// Draw the background colour
		}
    }
}
...
void DrawChar(int x, int y, int c, int mode) {					    // x and y are in pixels
	...
	if(fontNbr>=FIRST_UB_FONT) {
		DrawCharUB(x,y,c,mode);
		return;
	}
		...
		for(j = 0; j < fontWidth; j++)         						// Loop through each word (ie, horizontal scan line)
			for(k = 0; k < fontHeight; k++) {         				// Loop through the bits in the word (ie, vertical pixels)
				...
				for(h = 0; h < fontScale; h++)
					for(w = 0; w < fontScale; w++)
    				    if(t)
    				    	plotx(x + (j * fontScale) + w, y + (k * fontScale) + h, CurTxtFgColour);	// Draw the foreground colour
    				    else
    				    	plotx(x + (j * fontScale) + w, y + (k * fontScale) + h, CurTxtBgColour);	// Draw the background colour
			}
...
void initFont(void) {
	...
	//------------------------------------------------------------
	// Fonts from UB musst insertet after Fonts from MMBasic
	// (update NBRFONTS_IN_FLASH)
	//------------------------------------------------------------

	ftbl[3].p = C64_8x8_Table; // Font from from UB
	ftbl[3].width = 8;
	ftbl[3].height = 8;
	ftbl[3].start = ' ';
	ftbl[3].end = '~';

	ftbl[4].p = Arial_8x13b_Table; // Font from from UB
	ftbl[4].width = 8;
	ftbl[4].height = 13;
	ftbl[4].start = ' ';
	ftbl[4].end = '~';

	ftbl[5].p = Arial_10x15b_Table; // Font from from UB
	ftbl[5].width = 10;
	ftbl[5].height = 15;
	ftbl[5].start = ' ';
	ftbl[5].end = '~';
...
void UnloadFont(int font) {
	int bheight;
	if(font<NBRFONTS_IN_FLASH) return;
	if(ftbl[font].p != NULL) {
...
void ShowCursor(int show) {
	...
	if(sc == C_STANDARD)
		HBar(MMPosX - 1, MMPosY + fontHeight - 3, MMPosX + fontWidth - 1, MMPosY + fontHeight - 3, -1);	// invert the underline
	else
		VBar(MMPosX - x, MMPosY + fontHeight - 2, MMPosX - x, MMPosY, -1);	// invert the vert bar
