...
int item_init[MAX_NBR_OF_BTNS],item_touched[MAX_NBR_OF_BTNS],item_value[MAX_NBR_OF_BTNS], item_type[MAX_NBR_OF_BTNS],item_colour[MAX_NBR_OF_BTNS],
...
void reset_item(int index) {
	item_init[index] = 0;	// de-validize all items		
...
void cmd_touch(void) {
	...
	if((p = checkstring(cmdline, "ITEM CREATE")) != NULL || (p = checkstring(cmdline, "CREATE")) != NULL) {
	    getargs(&p, 13, ",");
		...
		reset_item(i);							// reset item vals
		item_init[i] = 0x55AA;					// validize item
		item_left[i] = x;
	...
	if((p = checkstring(cmdline, "ITEM REMOVE")) != NULL || (p = checkstring(cmdline, "REMOVE")) != NULL) {
    	int k;
    
    	getargs(&p, (MAX_ARG_COUNT * 2) - 1, ",");		                        // getargs macro must be the first executable stmt in a block

        if(checkstring(argv[0], "ALL")) {
		    for(i = 0; i <= MAX_NBR_OF_BTNS; i++) reset_item(i);
			return;
		}
	...
	if((p = checkstring(cmdline, "ITEM SIZE")) != NULL || (p = checkstring(cmdline, "SIZE")) != NULL) {
		getargs(&p, 3, ",");
	...
	error("Expected ITEM CREATE/REMOVE/SIZE, CHECK, RELEASE or WAIT");
}
