...
//--------------------------------------------------------------
static void P_BSP_COM_Init(UART_NAME_t uart, int fc);
...
//--------------------------------------------------------------
#if USE_UART1==1
  volatile uint8_t rx_buf1[RX_BUF_SIZE_COM1];
  volatile uint32_t rx_rd_ptr1;
  volatile uint32_t rx_wr_ptr1;
#endif
#if USE_UART6==1
  volatile uint8_t rx_buf6[RX_BUF_SIZE_COM6];
  volatile uint32_t rx_rd_ptr6;
  volatile uint32_t rx_wr_ptr6;
#endif
#if USE_UART7==1
  volatile uint8_t rx_buf7[RX_BUF_SIZE_COM7];
...
// init aller UARTs
//--------------------------------------------------------------
void UB_Uart_Init(int baud)
{
  #if USE_UART1==1 
    P_BSP_COM_Init(COM1, 0);

    // UART init
    UartHandle1.Instance = USART1;    
    UartHandle1.Init.BaudRate     = baud;
...
//--------------------------------------------------------------
// fuer MMBasic (init einer UART)
// uart : [COM6, COM7]
// baud : baudrate
// fc   : flowcontrol [0,1]
// s2   : stopbits [0=1, 1=2]
//--------------------------------------------------------------
void MM_Uart_Init(UART_NAME_t uart, int baud, int fc, int s2)
{
#if USE_UART6==1
  if(uart==COM6) {
  P_BSP_COM_Init(COM6, fc);

  UartHandle6.Instance = USART6;
  UartHandle6.Init.BaudRate     = baud;
  UartHandle6.Init.WordLength   = UART_WORDLENGTH_8B;
  if(s2) {
    UartHandle6.Init.StopBits     = UART_STOPBITS_2;
  }
  else {
  ...
  if(fc) {
    UartHandle6.Init.HwFlowCtl    = UART_HWCONTROL_RTS_CTS;
  }
  else {
...
#if USE_UART7==1
  if(uart==COM7) {
  P_BSP_COM_Init(COM7, fc);

  UartHandle7.Instance = UART7;
  UartHandle7.Init.BaudRate     = baud;
  UartHandle7.Init.WordLength   = UART_WORDLENGTH_8B;
  if(s2) {
    UartHandle7.Init.StopBits     = UART_STOPBITS_2;
  }
  else {
  ...
  if(fc) {
    UartHandle7.Init.HwFlowCtl    = UART_HWCONTROL_RTS_CTS;
  }
  else {
...
//--------------------------------------------------------------
// fuer MMBasic
// ein Byte vom UART Buffer auslesen
//--------------------------------------------------------------
int MM_Uart_ReveiveByte(UART_NAME_t uart)
{
  uint32_t anz;
  uint8_t wert;
  ...
  if(uart==COM1) {
    ...
    if(rx_wr_ptr1>=rx_rd_ptr1) {
      ...
      anz=(rx_wr_ptr1+RX_BUF_SIZE_COM1)-rx_rd_ptr1;
    }
    if(anz==0) return -1;
    ...
    rx_rd_ptr1++;
    if(rx_rd_ptr1>=RX_BUF_SIZE_COM1) rx_rd_ptr1=0;
  ...
  if(uart==COM6) {
    ...
    if(rx_wr_ptr6>=rx_rd_ptr6) {
      ...
      anz=(rx_wr_ptr6+RX_BUF_SIZE_COM6)-rx_rd_ptr6;
    }
    if(anz==0) return -1;
    ...
    rx_rd_ptr6++;
    if(rx_rd_ptr6>=RX_BUF_SIZE_COM6) rx_rd_ptr6=0;
  ...
  if(uart==COM7) {
    ...
    if(rx_wr_ptr7>=rx_rd_ptr7) {
      ...
      anz=(rx_wr_ptr7+RX_BUF_SIZE_COM7)-rx_rd_ptr7;
    }
    if(anz==0) return -1;
    ...
    rx_rd_ptr7++;
    if(rx_rd_ptr7>=RX_BUF_SIZE_COM7) rx_rd_ptr7=0;
...
//--------------------------------------------------------------
// fuer MMBasic
// anzahl der Daten im RX-Buffer auslesen
//--------------------------------------------------------------
int MM_Uart_GetRxStatus(UART_NAME_t uart)
{
  int anz;
...
static void P_BSP_COM_Init(UART_NAME_t uart, int fc)
{
  ...
  if(uart==COM7) {
    ...
    if(fc) { // RTS/CTS for MMBasic only on COM7
      // GPIO RTS
      gpio_init_structure.Pin = COM7_RTS_PIN;
      gpio_init_structure.Mode = GPIO_MODE_AF_PP;
      gpio_init_structure.Speed = GPIO_SPEED_FAST;
      gpio_init_structure.Pull = GPIO_PULLUP;
      gpio_init_structure.Alternate = GPIO_AF8_UART7;
      HAL_GPIO_Init(COM7_RTS_PORT, &gpio_init_structure);

      // GPIO CTS
      gpio_init_structure.Pin = COM7_CTS_PIN;
      gpio_init_structure.Mode = GPIO_MODE_AF_PP;
      gpio_init_structure.Alternate = GPIO_AF8_UART7;
      HAL_GPIO_Init(COM7_CTS_PORT, &gpio_init_structure);
    }
  }
}
...
void USART1_IRQHandler(void)
{
  ...
  if(__HAL_UART_GET_IT(&UartHandle1,UART_IT_RXNE)!=RESET) {
    ...
    rx_wr_ptr1++;
    if(rx_wr_ptr1>=RX_BUF_SIZE_COM1) rx_wr_ptr1=0;
  }
}
#endif
...
void USART6_IRQHandler(void)
{
  ...
  if(__HAL_UART_GET_IT(&UartHandle6,UART_IT_RXNE)!=RESET) {
    ...
    rx_wr_ptr6++;
    if(rx_wr_ptr6>=RX_BUF_SIZE_COM6) rx_wr_ptr6=0;
  }
}
#endif
...
void UART7_IRQHandler(void)
{
  ...
  if(__HAL_UART_GET_IT(&UartHandle7,UART_IT_RXNE)!=RESET) {
    ...
    rx_wr_ptr7++;
    if(rx_wr_ptr7>=RX_BUF_SIZE_COM7) rx_wr_ptr7=0;
  }
}
#endif
